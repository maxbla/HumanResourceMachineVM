...
impl OfficeTile {
    ...
    fn checked_sub(self, rhs: Self) -> Result<Self, ArithmeticError> {
        ...
        match (self, rhs) {
            (OfficeTile::Number(lhs), OfficeTile::Number(rhs)) => {
                let diff = lhs.checked_sub(rhs).ok_or_else(gen_error)?;
                OfficeTile::try_from(diff).map_err(|_| gen_error())
            }
            (OfficeTile::Character(lhs), OfficeTile::Character(rhs)) => {
                let diff = (lhs as i16).checked_sub(rhs as i16).ok_or_else(gen_error)?;
                OfficeTile::try_from(diff).map_err(|_| gen_error())
            }
            (lhs, rhs) => Err(ArithmeticError::TypeError(lhs, rhs)),
        }
...
impl fmt::Display for ArithmeticError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ArithmeticError::Overflow(lhs, rhs) => {
                write!(f, "Overflow Error between {} and {}", lhs, rhs)
            }
            ArithmeticError::TypeError(lhs, rhs) => {
                write!(f, "Type Error between {} and {}", lhs, rhs)
            }
        }
    }
}
